...
import { useLazyGetVotingRoomByIdQuery } from '@/api/getVotingRoomId';
...
export const DashBoardPage = () => {
  ...
  const [getVotingRoomById, { data, isLoading, error }] =
    useLazyGetVotingRoomByIdQuery();
  ...
  const handleJoinRoom = async () => {
    ...
    try {
      // 🔹 Step 1: Join room (POST)
      const joinResponse = await joinRoom({ roomCode }).unwrap();
      console.log('Room joined:', joinResponse);

      // 🔹 Step 2: Fetch voting room data (GET)
      const roomData = await getVotingRoomById(roomCode).unwrap();
      console.log('Voting room fetched:', roomData);

      // 🔹 Step 3: Navigate using voting room _id
      navigate({ to: `/voting-room/${roomData._id}` });
    } catch (err) {
      console.error('Error joining or fetching room:', err);
      alert(
        'Failed to join or fetch room. Please check the code or try again.'
      );
    }
  };
